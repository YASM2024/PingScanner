...
手動で実行することはもちろん、Windows タスクスケジューラから `main.exe` を毎日実行することで、定期的なPING監視することができます。
...
Python 3.8 以上が必要です。プロジェクトルートで `build.bat` を実行してください。

```bat
build.bat
```

成果物は `main.exe`、`export_csv.exe`、`remark_ui.exe` がプロジェクトルートに出力されます。
